/***************************************
  $Header: /home/amb/CVS/procmeter3/gtk2/run.c,v 1.1 2007-09-19 19:06:54 amb Exp $
...
void RunProgram(RunOption *run)
{
 ...
 if(run->flag==RUN_XBELL)
    XBell(GDK_DISPLAY_XDISPLAY(gtk_widget_get_display(toplevel)),0);
 else
   {
    pid_t pid=fork();
    ...
    if(pid==-1)
    ...
    else if(pid==0)
      {
       ...
       displayname=XDisplayString(GDK_DISPLAY_XDISPLAY(gtk_widget_get_display(toplevel)));
       ...
       close(ConnectionNumber(gtk_widget_get_display(toplevel)));
